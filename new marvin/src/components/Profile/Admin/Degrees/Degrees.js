...
class Degrees extends React.Component {
    ...
    render() {
        ...
        const empty = this.props.emptyDegrees ? < EmptyData label = 'no data found on blockchain' / > : < div / >
          ...
          return( <
              div > { load } { ipfsLoad } { empty } { /* {console.log('loadingAcademic: ' + this.props.loadingAcademic + '\nloadingDegree: ' + this.props.loadingDegree)} */ } {
                ...
                div >
                  ...
                  select type = "text"
                name = "years"
                value = { this.state.selectedYears } onChange = { this.onSelectChange } > { < option value = "select year"
                    disabled = { this.state.selectedYears === "" ? false : true } > --select an year-- < /option>} {
