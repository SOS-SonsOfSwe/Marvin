...
export default function addNewExam(classUnicode, examUnicode, examData) {
      ...
      web3.eth.getCoinbase((error, coinbase) => {
        ...
        admin.deployed()
          .then(instance => {
            ...
            ipfs.pushJSON(examData)
              .then(hashIPFS => {
                // dispatching ipfs data added
                dispatch(ipfsDataAdded)

                var examHash = ipfsPromise.getBytes32FromIpfsHash(hashIPFS)
                ...
                adminIstance.addNewExam(classUnicode, examUnicode, examHash, { from: coinbase })
                  ...
                  .finally(def => {
                    return browserHistory.push('/profile/exams')
                  })
              })
              .catch(err => {
